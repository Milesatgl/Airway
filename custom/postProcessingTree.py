...
REMOVE_IF_GROUP_SIZE_LESS_THAN or less. 

Recoloring for example
...
from composeTree import set_attribute_to_node, set_level
...
def calc_diameter(area):
    # (d / 2) ** 2 * math.pi = area -> d**2 = area*4/math.pi
    return math.sqrt(4 * area / math.pi)


def distance(graph: nx.Graph, node_a: str, node_b: str):
...
def remove_nodes(graph: nx.Graph, nodes_to_be_removed: List[str]):
...
def combine_group_sizes(graph: nx.Graph, node_pre: str, node: str, node_suc: str):
...
def merge_edges(graph: nx.Graph, predecessor: str, node: str, successor: str):
...
def assign_children_count(graph: nx.Graph):
    ...
    def successor_count(curr_node="0"):
        if curr_node in successors:
            # 该节点有后继节点，递归计算每个后继节点的后继节点数量并累计总和
...
# def get_successor_lobes(graph: nx.Graph, return_count=False):
...
def set_attribute_recursively(graph: nx.Graph, node, attribute_name, value):
...
def remove_minor_edges(graph: nx.Graph):
    ...
    for fr, to in nx.bfs_edges(graph, "0"):
        ...
        if to not in has_successors:
            ...
            node_diameter = graph.nodes.data()[fr]["group_size"] # 这个是直径
            avg_edge_length = graph[fr][to]["group_sizes"].count(" ") # 这个是段长
            # 段长接近直径时就移除终点 to
...
def straighten_edges(graph: nx.Graph):
    """Merges all nodes which only have 1 child with their parent"""
    predecessors = dict(nx.bfs_predecessors(graph, "0")) # 这里包括叶节点，但是不包括"0"
    # successors = dict(nx.bfs_successors(graph, "0"))
    # 这里面是不包括叶节点的？因为叶节点已经没有后继节点了
    # successors is {
    # '0': ['1'], 
    # '1': ['2', '7'], 
    # '2': ['3', '10'], 
    ...
    # '5': ['6', '18'],
    # ...}
    only_single_successor: List[Tuple[str, str]] = [(n, *s) for n, s in nx.bfs_successors(graph, "0") if len(s) == 1]
    nodes_to_be_removed = []
    cant_be_removed = {"0"} # 保留起始点
    for node, successor in only_single_successor:
        if node not in cant_be_removed:
            ...
            cant_be_removed.add(successor) # 合并之后的节点就不能被删除了
    remove_nodes(graph, nodes_to_be_removed)


def merge_close_nodes(graph: nx.Graph):
    ...
    for predecessor, node in nx.bfs_edges(graph, "0"):
        if node not in cant_be_removed:
            curr = graph[predecessor][node] # current edge,与上一节点的连线
            nums = list(map(int, curr["group_sizes"].split())) # 长度代表段长（曼哈顿距离？），元素代表代表结尾和起始的平均面积
            weight = curr["weight"] # 代表两点空间坐标的欧氏距离
            diameter = calc_diameter(sum(nums) / len(nums)) # sum(nums) / len(nums) 是整段的平均面积
            # 也是拿 空间两点距离 和 段的直径作比较
            if weight < diameter * DIAMETER_TO_WEIGHT_RATIO:
                if node in has_successors:
                    for successor in has_successors[node]:
                        cant_be_removed.add(successor) # 保留 successor
                        # 就是删除了中间节点
...
def remove_children_without_children(graph: nx.Graph):
        ...
        for node in nodes_to_check:
            if node in successors:
                for succ in successors[node]:
                    # 对于每一个node，添加node的下一级
                    new_nodes.add(succ)
        # Unify sets
        nodes_to_check |= new_nodes # 原位或
    ...
    if nodes_to_be_removed:
        print(f"Found {len(nodes_to_be_removed)} in top 3 layers to remove as they don't have any children")
    remove_nodes(graph, nodes_to_be_removed)
...
def postProcessingTree(input_data_path: Path):
    """Executes all methods given above in the correct order"""
    # |>-<-><-><-><-><-><-><-<|
    # |>- Process arguments -<|
    # |>-<-><-><-><-><-><-><-<|

    output_data_path = input_data_path.parent
    # input_data_path = Path(r'')

    # |>-<-><-><-><->-<|
    # |>- Load graph -<|
    # |>-<-><-><-><->-<|

    graph = load_graph(input_data_path)
    ...
    nx.write_graphml(graph, output_data_path / "S4_postProcessingTree.graphml")
...
if __name__ == "__main__":
    import sys
    projDir = sys.argv[1]
    # inputTreePath = Path(r'E:\conda_projects\Airway\data\CY_14210425\S3_composeTree.graphml')
    inputTreePath = Path(projDir).joinpath('S3_composeTree.graphml')
    postProcessingTree(input_data_path=inputTreePath)

# "D:\mambaforge\envs\skeletonA\python.exe" E:\conda_projects\Airway\custom\splitClassification.py E:\conda_projects\Airway\data\UN_10122643T
...
# def recolor_if_all_adjacent_have_different_color(graph: nx.Graph):
...
# def possibly_make_neutral_above_level_4(graph: nx.Graph):
...
# def recolor_if_successors_all_different_color(graph: nx.Graph):
...
# def add_new_parent_for_lobe(graph: nx.Graph):
...
# def recolor_entire_subtree_to_majority_at_level_4_or_5(graph: nx.Graph):
